...
import com.bc.spring.thymeleaf.study.common.entity.account.RoleType;
...
public class RoleTypeHandler extends BaseTypeHandler<RoleType> {
    private Class<RoleType> type;

    private final RoleType[] enums;

    /**
     * 设置配置文件设置的转换类以及枚举类内容，供其他方法更便捷高效的实现
     * @param type 配置文件中设置的转换类
     */
    public RoleTypeHandler(Class<RoleType> type) {
        if (type == null)
     ...
     * @return code对应的枚举类
     */
    private RoleType locateEnumStatus(int code) {
        for(RoleType status : enums) {
